...
if (isset($_GET['id'])) {
    ...
    {
        $userId = (int)$_GET['id']; // Ensure that the id is an integer to prevent SQL injection
        $stmt = $conn->prepare("SELECT * FROM `faculty_list` WHERE id = ?");
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    {
        $userId = (int)$_GET['id']; // Ensure that the id is an integer to prevent SQL injection
        $stmt = $conn->prepare("SELECT * FROM `superior_list` WHERE id = ?");
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $result = $stmt->get_result();
