...
<script>
    document.title = "Questions (Superiors) | Administrator";
</script>
<h1 class="m-0 text-center"><i class="fas fa-question-circle"></i>&nbsp;<b> QUESTIONS LIST (SUPERIOR)</b></h1><br><hr>
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="callout" style="border: 0px;">
                            <?php 
                                $qry = $conn->query("SELECT * FROM questions_list_superior");
                                if($qry->num_rows > 0):
                            ?>
                            <div class="justify-content-center w-40" style="font-size:25px;">
                                <label for=""><b>Questions</b></label>
                                <input type="text" id="searchQuestion" class="form-control mt-2" placeholder="Search question...">
                            </div>
                            <hr>
                    ...
                    <div class="col-md-4 ">
                        <div class="form-group mb-1 d-flex" >
                            <form method="post" id = "import_questions_superior"enctype="multipart/form-data" class="ml-1">
...
<script>
    ...
    $('#import_questions_superior').submit(function(event) {
        event.preventDefault(); // Prevent default form submission
        var formData = new FormData(this);
        start_load()
        $.ajax({
            url: 'ajax.php?action=import_questions_superior',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).ready(function(){
        ...
        $('.edit_question').click(function(){
            ...
            start_load()
            $.ajax({
                url: 'ajax.php?action=get_question_superior',
                method: 'POST',
                dataType: 'json',
                data: {id: id},
                success: function(response) {
        ...
        $('#manage-question').submit(function(e){
            ...
            start_load()
            $.ajax({
                url: 'ajax.php?action=save_questions_superior',
                method: 'POST',
                data: formData,
                success:function(response){
                    if(response == 1){
    ...
    function delete_question(id){
        ...
        start_load()
        $.ajax({
            url: 'ajax.php?action=delete_questions_superior',
            method: 'POST',
            data: {id: id},
            success:function(response){
                if(response == 1){
