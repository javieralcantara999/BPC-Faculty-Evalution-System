...
$questions = $conn->query("SELECT * FROM question_list_superior");
...
<h1 class="m-0 text-center"><i class = "fas fa-file-alt"></i>&nbsp;<b>MANAGE QUESTIONS (SUPERIORS)</b></h1><br><hr>
					...
					<form action="" id="manage-question">
						...
						<div class="form-group">
							...
							<select name="criteria_id" id="criteria_id" class="custom-select custom-select-sm select2">
								<option value=""></option>
							<?php 
								$criteria = $conn->query("SELECT * FROM criteria_list_superior order by abs(order_by) asc ");
						...
						<div class="form-group">
                            ...
                            <select name="question" id="question_id" class="custom-select custom-select-sm select2">
                                <option value=""></option>
                                <?php 
                                $questions = $conn->query("SELECT * FROM questions_list_superior");
                                while ($row = $questions->fetch_assoc()) {
...
while ($crow = $criteria->fetch_assoc()) {
    $criteria_id = $crow['id'];
    $questions = $conn->query("SELECT * FROM question_list_superior WHERE criteria_id = $criteria_id AND academic_id = $id ORDER BY abs(order_by) ASC");
    ?>
                    ...
                    <td class="p-1 text-center" width="5px">
                        <span class="btn-group dropright">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item dropdown-item-success delete_question" href="javascript:void(0)" 
...
<script>
    ...
    $.ajax({
    url: 'ajax.php?action=save_question_superior',
    data: formData,
    cache: false,
    contentType: false,
    processData: false,
    ...
    $('#order-question').submit(function(e){
        ...
        $.ajax({
            url: 'ajax.php?action=save_question_order_superior',
    ...
    start_load();
    $.ajax({
        url: 'ajax.php?action=delete_question_superior',
        method: 'POST',
        data: {id: id},
        success: function(resp) {
            if(resp == 1) {
